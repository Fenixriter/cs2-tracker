...
# --- ФУНКЦИЯ ЗАЩИТЫ (ПРОГРАММНЫЙ ПИН-КОД) ---
def check_password():
    """Возвращает True, если пользователь ввел правильный пароль."""
    if "password_correct" not in st.session_state:
        st.session_state["password_correct"] = False

    # Если уже вошли, сразу пускаем
    if st.session_state["password_correct"]:
        return True

    # Форма ввода пароля
    st.title("🔒 Доступ ограничен")
    password = st.text_input("Введите секретный пароль для доступа к инвентарю:", type="password")
    
    # ⚠️ ВНИМАНИЕ: Замените 'MySecret2026' на свой сложный пароль!
    if st.button("Войти"):
...
# Запускаем проверку безопасности. Если пароль не введен, код дальше не выполняется!
if check_password():
    ...
    with col1:
        st.metric(label="ИНВЕСТИРОВАНО (ВСЕГО)", 
